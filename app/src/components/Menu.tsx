...
export default function Menu({pathname, menuImage}: {pathname:string, menuImage: any}) {
  ...
  const [navigation] = React.useState([
    ...
    { name: 'Schedule', href: '/schedule', id: 'schedule-menu-href'},
    ...
    { name: 'FAQ', href: '/faq', id: 'faq-menu-href'},
    ...
    <header className="w-full border-b border-slate-500 ">
      ...
      {navigation.find(item => item.href === pathname)?.name}
        ...
        <div className="hidden md:flex justify-center md:gap-x-12">
          {navigation.map((item) => (
            <a key={item.name} id={item.id} href={item.href} className={classNames("text-sm  leading-6 text-gray-900", pathname === item.href && 'font-bold')}>
              {item.name}
            </a>
          ))}
        </div>
